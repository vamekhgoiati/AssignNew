...
public class Assign2 implements EntryPoint {
	...
	private void initializeListeners() {
		...
		FITypeList.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				rowIndexForType = FITypeList.getCellForEvent(event).getRowIndex();
				...
				AsyncCallback<Institution> callback = new AsyncCallback<Institution>() {
					...
					public void onSuccess(Institution result) {
						...
						for(int i = 0; i < FITypeChooser.getItemCount(); i++){
							if(FITypeChooser.getItemText(i).equals(result.getType().getCode())){
						...
						if(result.getRegDate() != null){
							...
							FIRegDateText.setText(myDateTimeFormat.format(result.getRegDate()));
						} else
		...
		addFIType.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(FITypeNameText.getText().equals(""))
					message = message + ((message.equals(""))? "You must enter the name!" : "\nYou must enter the name!");
				if(!FieldVerifier.isValidCode(FITypeCodeText.getText()))
					message = message + ((message.equals(""))? "Invalid code!" : "\nInvalid code");
				if(!FieldVerifier.isValidName(FITypeNameText.getText()))
					message = message + ((message.equals(""))? "Invalid name!" : "\nInvalid name");
				if(message.equals("")){
					createFIType();
				} else {
		...
		addFI.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if(FIRegDateText.getText().equals(""))
					message = message + ((message.equals(""))? "You must enter the date!" : "\n You must enter the date!");
				...
				if(!FieldVerifier.isValidPhone(FIPhoneText.getText()))
					message = message + ((message.equals(""))? "Invalid phone!" : "\n Invalid phone");
				if(!FieldVerifier.isValidEmail(FIEmailText.getText()))
					message = message + ((message.equals(""))? "Invalid email!" : "\n Invalid email");
				if(!FieldVerifier.isValidFax(FIFaxText.getText()))
					message = message + ((message.equals(""))? "Invalid fax!" : "\n Invalid fax");
				if(!FieldVerifier.isValidDate(FIRegDateText.getText()))
					message = message + ((message.equals(""))? "Invalid date!" : "\n Invalid date");		
				if(message.equals("")){
					createFI();
				} else {
					infoLabel.setText(message); 
	...
	private void updateFIType() {
		InstitutionType type = new InstitutionType(FITypeCodeText.getText(), FITypeNameText.getText());
	...
	private void checkFIByCode(String code) {
		AsyncCallback<Institution> callback = new AsyncCallback<Institution>() {
			...
			public void onSuccess(Institution result) {
				if(result != null){
					infoLabel.setText("FI with such code is already registered!");
					FIRegistered = true;
				} else {
					getType(FITypeChooser.getItemText(FITypeChooser.getSelectedIndex()), 0);
				}
			}
	...
	private void checkFITypeByCode(String code) {
		AsyncCallback<InstitutionType> callback = new AsyncCallback<InstitutionType>() {
			...
			public void onSuccess(InstitutionType result) {
				if(result != null){
					infoLabel.setText("FI type with such code is already registered!");
		...
		AsyncCallback<Integer> callback = new AsyncCallback<Integer>() {
			...
			public void onSuccess(Integer result) {
				if(result == 0){
					...
					infoLabel.setText("Current FI Type with \"Code\" = " + code + " is used and can't be deleted");
				}
			}
	...
	private void getType(String code, final int mode) {
		AsyncCallback<InstitutionType> callback = new AsyncCallback<InstitutionType>() {
			...
			public void onSuccess(InstitutionType result) {
				if(result != null){
					...
					if(!FIRegDateText.getText().equals("")){
						try {
							DateTimeFormat myDateTimeFormat = DateTimeFormat.getFormat("dd/MM/yyyy");
							regDate = myDateTimeFormat.parseStrict(FIRegDateText.getText());
					...
					Institution inst = new Institution(FICodeText.getText(), FINameText.getText(), FIAddressText.getText(), FIPhoneText.getText(), FIEmailText.getText(), FIFaxText.getText(), result, regDate);
					AsyncCallback<Boolean> callback = new AsyncCallback<Boolean>() {
						...
						public void onSuccess(Boolean result) {
							if(result){
								...
								else infoLabel.setText("FI updated!");
								fillFIList();
							}
						}
					...
					else FIManager.updateInstitution(inst, callback);
				}
			}
